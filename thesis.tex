...
\cleardoublepage
\chapter*{Abstract}
Through the popularity of cryptocurrency, Blockchain technologies have gained a lot of traction. These technologies play a huge part in the Web 3.0, as they are able to make decentralised applications possible. The Web by itself is decentralized, because there are many different servers/companies, that offer services to their users. However, big, centralized service providers, such as Google still dominate the Web. Therefore, the aforementioned companies have got a lot of user data in their hands. It can be argued, that a single entity/company, holding such a big quantity of user data, can influence their customers, by using their personal information. People may feel insecure about not knowing how their data is handled and who has access to it. The decentralization of the Web could improve upon the privacy of users, by offering Blockchain based applications, where users can either allow or deny requests to their personal data.\\
Blockchain technologies could improve the way data is handled in many different fields, such as the energy sector, the healthcare sector, voting and identity management. However there are still challenges that need to be overcome, before many of these applications become possible. One of these challenges is the energy consumption of Blockchains, which can be drastically improved, by switching from the Proof of Work (PoW) to the Proof of Stake (PoS) algorithm. Other challenges are the trade off between security, performance and scalability, storing data on the Blockchain, interoperability between different Blockchain technologies and fairness and security in Blockchain networks. \\
This Bachelor thesis combines many different papers, that deal with the challenges and opportunities of the Web 3.0/Blockchain technologies. Some papers were taken as a point of reference and were expanded with more recent discoveries from other papers and different perspectives.


% --- German Abstract ----------------------------------------------------
\cleardoublepage
\chapter*{Kurzfassung}
Durch die Popularität von Kryptowährungen haben Blockchain Technologien viel Aufmerksamkeit erlangt. Diese Technologien spielen eine große Rolle im Web 3.0, da sie dezentrale Applikationen möglich machen. Das Web selbst ist dezentral, da es viele verschiedene Server/Firmen gibt, die ihre Dienste Nutzern zur Verfügung stellen. Jedoch dominieren große, zentrale Dienstanbieter, wie zum Beispiel Google das Web. Diese Dienstanbieter haben dadurch Zugriff auf eine große Menge an Nutzerdaten. Es kann argumentiert werden, dass eine einzelne Entität/Firma, die eine große Menge an Nutzerdaten hat, Ihre Benutzer beeinflussen kann. Benutzer könnten sich damit unsicher fühlen, dass sie nicht genau wissen, was mit ihren Daten passiert und wer Zugriff darauf hat. Die Dezentralisierung des Web könnte die Privatsphäre von Nutzern, durch den Einsatz von Blockchain Technologien, mit denen Nutzer entscheiden können, wer Zugriff auf ihre Daten hat, verbessern.\\ Blockchain Technologien könnten die Art, wie mit Daten umgegangen wird in vielen Bereichen, wie zum Beispiel dem Energiebereich, dem Gesundheitsbereich, bei Wahlen, und dem Identitäts-management, verbessern. Bevor viele von diesen Applikationen umgesetzt werden können, gibt es aber noch einige Herausforderungen, die bewältigt werden müssen. Eine dieser Herausforderungen ist der Energieverbrauch von Blockchains, der durch den Wechsel von einem Proof of Work (PoW) Algorithmus, zu einem Proof of Stake (PoS) Algorithmus, drastisch verbessert werden kann. Andere Herausforderungen sind, zum Beispiel, der Kompromiss zwischen Sicherheit, Leistung und Skalierbarkeit, das Speichern von Daten auf der Blockchain, die Kompatibilität zwischen verschiedenen Blockchain Technologien und Fairness und Sicherheit in Blockchain Netzwerken. \\
Diese Bachelorarbeit kombiniert viele verschiedene Werke, die sich mit den Herausforderungen und Möglichkeiten von Web 3.0/Blockchain Technologien auseinandersetzen. Manche Werke wurden dabei als Referenzpunkt verwendet und mit Erkenntnissen von neueren Werken erweitert, sowie mit verschiedenen Blickwinkeln.
...
\newpage
\chapter{Web 3.0}
The first question, that emerges, when talking about the Web 3.0 (also called the Web3), is what it is by definition. There is neither an easy, nor a precise answer to that question, since there is no clear definition of the term at the time\cite{techtarget}. To better understand what the Web 3.0 is and what the term stands for, we can analyse the history of the two previous iterations of the Web, the Web 1.0 and Web 2.0.

\section{History of the Web}
First of, it is important to understand, that the transformation of the Web 1.0 to the Web 2.0 did not take place at a concrete time. It was rather a slow progress, where certain websites gradually implemented additional functionalities and technologies, eg. AJAX. \cite{oreilly2007}\\
...
The decentralized part of the Blockchain is then made possible by people who provide their own network bandwidth and computational power to the network. These people are called miners and represent the nodes of a network. Miners have the responsibility of adding new blocks to the Blockchain. In the case of cryptocurrency, they are therefore needed to validate transactions in the network. As a reward for the participation of miners, they get a certain amount of the cryptocurrency e.g. Ethereum, whenever a new block is created. The process of validating the Blockchain is called the consensus process, in a public Blockchain, each user can participate in this process.\\
The first well known consensus strategy, the Proof of Work (PoW) strategy, is also used in the Bitcoin network. This requires complex mathematical calculations to be done, which also creates a number of problems e.g. energy cost, that will be addressed in chapter 3.\\
...
\newpage
\chapter{Conclusion}
The Web is continually growing and new technologies emerge often. There have been many changes since the Web 1.0 was first released. The current Web 2.0 focuses on "rich user experiences" and social media. Here the user is the center of the Web, with the ability to create and consume content. This Bachelor thesis has shown, that there is no clear definition for the Web 3.0 as of yet. However, it is clear, that decentralization, immutability and privacy play a huge part in it. These properties of the next generation of the Web can be achieved with Blockchain technologies.\\
This thesis has further shown what some of the challenges, that the Web 3.0 has yet to face are and how to potentially solve them. The energy consumption of Blockchain technologies can be greatly reduced by switching from the PoW to the PoS algorithm, which has already been implemented by the Ethereum network. The challenge of managing personal data, could be solved by implementing access and data transactions on a Blockchain network. The interoperability between Blockchain networks could be improved with the "HyperService" platform. The challenges that still remain are the trade off between scalability, security and decentralisation as well as problems with fairness and security in Blockchain networks.\\
The opportunities for Blockchain applications, that this Bachelor thesis has presented can be found in the energy sector, the healthcare sector, the management of the voting process and identity management. In the energy sector Blockchain technologies could improve the management of a distributed market. In the healthcare sector they could be used to manage patient data and provide faster access to this data, as well as a way to ensure the trustworthiness of data in the field of medical research. Blockchain technologies could improve the voting process, by making it more resistant against voting fraud. They could be helpful to users, by providing them with a more secure way of managing their identity.\\
These are of course just some of the challenges and applications for Blockchain technologies. It remains to be seen, if these applications are ever perfected and gain popularity among the users of the Web.
